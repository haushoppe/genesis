...
contract testSuite {
    ...
    function shouldSupportAllInterfaces() public {
        ...
        bytes4 lendableInterfaceId2 = 0xcd36757f;

        Assert.equal(lendableInterfaceId, lendableInterfaceId2, "ILendable should have this interfaceId: 0xcd36757f");
        Assert.equal(token.supportsInterface(0xcd36757f), true, "Contract should support ILendable: 0xcd36757f");
    ...
    // in gift() we removed the parenthesis, just to be sure that it really doesn't matter
    function checkMathematicalOperationsOrder() public {
        ...
        Assert.equal(result2, 19, "with parenthesis: 19");
    }
}
